...
export default function PlayersList() {
  ...
  const {
    ...
    queryKey: ['players'],
    queryFn: ({ pageParam = "" }) => getPlayersFn({ take: 10, lastCursor: pageParam }),
    getNextPageParam: (lastPage) => {
      return lastPage?.metaData.lastCursor;
    }
  });

  console.error(data?.pageParams, data?.pages);
  ...
  return (
    <>
      <h3 className="text-2xl mt-4">Players List</h3>
      <div className="mt-10 w-2/3">
        ...
        {(isLoading || isFetchingNextPage) && <div className="flex justify-center"><Loading /></div>}
      </div>
    </>
  );
}